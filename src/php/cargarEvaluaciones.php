<?php 
	...
	if ($fecha AND $descripcion AND $plan AND $materia) {
		...
		$ins = $myPDO2->prepare("
			INSERT INTO actividades(id_profesorcursogrupo, id_plan_evaluacion, publicacion, fecha, descripcion,nlink1,nlink2,nlink3,nlink4,link1,link2,link3,link4)
			VALUES($materia, $plan, '$publicado', '$fecha', '$descripcion','$nlink1','$nlink2','$nlink3','$nlink4','$link1','$link2','$link3','$link4');
			");
		$ins->execute();
		...
		$se = $myPDO2->prepare("
			...
			AND id_plan_evaluacion = $plan 
			AND publicacion = '$publicado'
			AND fecha = '$fecha'
...
##################### ARCHIVO 1 ################
	if ($_FILES['file1']) {
		...
		$extensiones = array('pdf','doc','docx','xlsx','xls','txt','pptx','ppt','pub','jpg','jpeg','gif','png');
