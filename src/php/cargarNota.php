<?php 
	require_once('conn.php');
	$materia = (int)$_POST['materia'];
	$plan = (int)$_POST['plan'];
	$alumno = (int)$_POST['alumno'];
	$toggle = (int)$_POST['toggle'];
	$evalu = (int)$_POST['evalu'];
	
	$nota = (int)$_POST['nota'];
	$observacion = $_POST['observacion'];


	
	

	if(is_numeric(!$materia)){
		print "MATERIA ERROR <br/>";
	}

	

	$sel = $myPDO->prepare("
		SELECT 
		i.id_estudia, 
		e.cedula, 
		e.p_nombres, 
		e.p_apellido  
		FROM inscripcion i
		inner join estudiante e on i.id_estudia = e.id_estudia 
		WHERE 
		i.id_profesorcursogrupo = $materia AND
		i.id_estudia = $alumno 
		");
	$sel->execute();
	$vAlumno = $sel->fetch();


	



	if ($vAlumno && $nota >= 0 && $nota <= 20) {
		
		$preve = $myPDO2->prepare("
			SELECT * FROM notas 
			WHERE id_plan_evaluacion = $plan AND 
			id_estudiante = $alumno 
		");
		$preve->execute();
		$prevenir = $preve->fetch();
		
		if (!$prevenir) {
			
			$insertar = $myPDO2->prepare("
				INSERT INTO notas (id_estudiante, id_plan_evaluacion, nota, observacion)
				VALUES($alumno, $plan, $nota, '$observacion');
				");
			$insertar->execute();
			
		}else{
			$update = $myPDO2->prepare("
					UPDATE notas 
					SET 
					nota = $nota,
					observacion = '$observacion' 
					WHERE 
					id_estudiante = $alumno AND
					id_plan_evaluacion = $plan 
				");
			$update->execute();
		}

		


	}




##################### ARCHIVO 1 ################
	if ($_FILES['file1']) {
		
		$tmp = $_FILES['file1']['tmp_name'];
		$size = $_FILES['file1']['size'];
		$tip = explode(".", $_FILES['file1']['name']);
		$tipo = end($tip);
		$nombre = "Rev-$alumno-1";
		$limite = 20480 *1024;

		$ruta = "../../upload/correcciones/$materia/$plan/";


		$extensiones = array('pdf','doc','docx','xlsx','xls','txt','pptx','ppt','pub','jpg','jpeg','gif','png','ai','svg','git','psd','raw','mp4','m4v','mov','mpg','mpeg','swf','zip','rar','mp3','wav','opus','PDF','DOC','DOCX','XLSX','XLS','TXT','PPTX','PPT','PUB','JPG','JPEG','GIF','PNG','AI','SVG','GIT','PSD','RAW','MP4','M4V','MOV','MPG','MPEG','SWF','ZIP','RAR','MP3','WAV','OPUS','Pdf','Doc','Docx','Xlsx','Xls','Txt','Pptx','Ppt','Pub','Jpg','Jpeg','Gif','Png','Ai','Svg','Git','Psd','Raw','Mp4','M4V','Mov','Mpg','Mpeg','Swf','Zip','Rar','Mp3','Wav','Opus','docm','dotx','dotm','dot','xps','rtf','xml','odt','xlsm','xltx','xltm','xlsb','xlam','xls','xlt','csv','prm','dif','xlam','xla','xps','ods','pptx','pptm','potx','potm','ppam','ppsx','ppsm','sldx','sldm','thmx','ppt','xps','pot','thmx','','pps','ppa','wmv','tif','bmp','wmf','emf','rtf','odp','pub','ps','xps');

		if($size <= $limite){
			if(!file_exists($ruta)){
				mkdir($ruta,0777,true);
			}
			foreach ($extensiones as $ext) {
					if (is_file("$ruta$nombre.$ext")) {
						unlink("$ruta$nombre.$ext");
						
					}
				}
			if (move_uploaded_file($tmp, $ruta.$nombre.".".$tipo)) {
				$update1 = $myPDO2->prepare("
			   		UPDATE notas SET file1='$nombre.$tipo' 
					WHERE id_estudiante = $alumno AND id_plan_evaluacion = $plan;
			   		");
			   	$update1->execute();
				
				#print "ARCHIVO GUARDADO";
			}else{
				#print "NO SE GUARDO EL ARCHIVO";
			}
		}
		
	}
	



##################### ARCHIVO 2 ################
	if ($_FILES['file2']) {
		$tmp = $_FILES['file2']['tmp_name'];
		$size = $_FILES['file2']['size'];
		$tip = explode(".", $_FILES['file2']['name']);
		$tipo = end($tip);
		$nombre = "Rev-$alumno-2";
		$limite = 20480 *1024;

		$ruta = "../../upload/correcciones/$materia/$plan/";

		$extensiones = array('pdf','doc','docx','xlsx','xls','txt','pptx','ppt','pub','jpg','jpeg','gif','png','ai','svg','git','psd','raw','mp4','m4v','mov','mpg','mpeg','swf','zip','rar','mp3','wav','opus','PDF','DOC','DOCX','XLSX','XLS','TXT','PPTX','PPT','PUB','JPG','JPEG','GIF','PNG','AI','SVG','GIT','PSD','RAW','MP4','M4V','MOV','MPG','MPEG','SWF','ZIP','RAR','MP3','WAV','OPUS','Pdf','Doc','Docx','Xlsx','Xls','Txt','Pptx','Ppt','Pub','Jpg','Jpeg','Gif','Png','Ai','Svg','Git','Psd','Raw','Mp4','M4V','Mov','Mpg','Mpeg','Swf','Zip','Rar','Mp3','Wav','Opus','docm','dotx','dotm','dot','xps','rtf','xml','odt','xlsm','xltx','xltm','xlsb','xlam','xls','xlt','csv','prm','dif','xlam','xla','xps','ods','pptx','pptm','potx','potm','ppam','ppsx','ppsm','sldx','sldm','thmx','ppt','xps','pot','thmx','','pps','ppa','wmv','tif','bmp','wmf','emf','rtf','odp','pub','ps','xps');

		if($size <= $limite){
			foreach ($extensiones as $ext) {
					if (is_file("$ruta$nombre.$ext")) {
						unlink("$ruta$nombre.$ext");
						
					}
			}
			if(!file_exists($ruta)){
				mkdir($ruta,0777,true);
			}
			if (move_uploaded_file($tmp, $ruta.$nombre.".".$tipo)) {
				$update2 = $myPDO2->prepare("
			   		UPDATE notas SET file2='$nombre.$tipo' 
					WHERE id_estudiante = $alumno AND id_plan_evaluacion = $plan;
			   		");
			   	$update2->execute();
				#print "ARCHIVO GUARDADO";
			}else{
				#print "NO SE GUARDO EL ARCHIVO";
			}
		}
		
	}




##################### ARCHIVO 3 ################
	if ($_FILES['file3']) {
		$tmp = $_FILES['file3']['tmp_name'];
		$size = $_FILES['file3']['size'];
		$tip = explode(".", $_FILES['file3']['name']);
		$tipo = end($tip);
		$nombre = "Rev-$alumno-3";
		$limite = 20480 *1024;

		$ruta = "../../upload/correcciones/$materia/$plan/";


		$extensiones = array('pdf','doc','docx','xlsx','xls','txt','pptx','ppt','pub','jpg','jpeg','gif','png','ai','svg','git','psd','raw','mp4','m4v','mov','mpg','mpeg','swf','zip','rar','mp3','wav','opus','PDF','DOC','DOCX','XLSX','XLS','TXT','PPTX','PPT','PUB','JPG','JPEG','GIF','PNG','AI','SVG','GIT','PSD','RAW','MP4','M4V','MOV','MPG','MPEG','SWF','ZIP','RAR','MP3','WAV','OPUS','Pdf','Doc','Docx','Xlsx','Xls','Txt','Pptx','Ppt','Pub','Jpg','Jpeg','Gif','Png','Ai','Svg','Git','Psd','Raw','Mp4','M4V','Mov','Mpg','Mpeg','Swf','Zip','Rar','Mp3','Wav','Opus','docm','dotx','dotm','dot','xps','rtf','xml','odt','xlsm','xltx','xltm','xlsb','xlam','xls','xlt','csv','prm','dif','xlam','xla','xps','ods','pptx','pptm','potx','potm','ppam','ppsx','ppsm','sldx','sldm','thmx','ppt','xps','pot','thmx','','pps','ppa','wmv','tif','bmp','wmf','emf','rtf','odp','pub','ps','xps');

		if($size <= $limite){
			foreach ($extensiones as $ext) {
					if (is_file("$ruta$nombre.$ext")) {
						unlink("$ruta$nombre.$ext");
						
					}
			}
			if(!file_exists($ruta)){
				mkdir($ruta,0777,true);
			}
			if (move_uploaded_file($tmp, $ruta.$nombre.".".$tipo)) {
				$update3 = $myPDO2->prepare("
			   		UPDATE notas SET file3='$nombre.$tipo' 
					WHERE id_estudiante = $alumno AND id_plan_evaluacion = $plan;
			   		");
			   	$update3->execute();
				#print "ARCHIVO GUARDADO";
			}else{
				#print "NO SE GUARDO EL ARCHIVO";
			}
		}
		
	}

##################### ARCHIVO 4 ################
	if ($_FILES['file4']) {
		$tmp = $_FILES['file4']['tmp_name'];
		$size = $_FILES['file4']['size'];
		$tip = explode(".", $_FILES['file4']['name']);
		$tipo = end($tip);
		$nombre = "Rev-$alumno-4";
		$ruta = "../../upload/correcciones/$materia/$plan/";
		$limite = 20480 *1024;

		
		

		$extensiones = array('pdf','doc','docx','xlsx','xls','txt','pptx','ppt','pub','jpg','jpeg','gif','png','ai','svg','git','psd','raw','mp4','m4v','mov','mpg','mpeg','swf','zip','rar','mp3','wav','opus','PDF','DOC','DOCX','XLSX','XLS','TXT','PPTX','PPT','PUB','JPG','JPEG','GIF','PNG','AI','SVG','GIT','PSD','RAW','MP4','M4V','MOV','MPG','MPEG','SWF','ZIP','RAR','MP3','WAV','OPUS','Pdf','Doc','Docx','Xlsx','Xls','Txt','Pptx','Ppt','Pub','Jpg','Jpeg','Gif','Png','Ai','Svg','Git','Psd','Raw','Mp4','M4V','Mov','Mpg','Mpeg','Swf','Zip','Rar','Mp3','Wav','Opus','docm','dotx','dotm','dot','xps','rtf','xml','odt','xlsm','xltx','xltm','xlsb','xlam','xls','xlt','csv','prm','dif','xlam','xla','xps','ods','pptx','pptm','potx','potm','ppam','ppsx','ppsm','sldx','sldm','thmx','ppt','xps','pot','thmx','','pps','ppa','wmv','tif','bmp','wmf','emf','rtf','odp','pub','ps','xps');

		if($size <= $limite){
			foreach ($extensiones as $ext) {
					if (is_file("$ruta$nombre.$ext")) {
						unlink("$ruta$nombre.$ext");
						
					}
			}
			if(!file_exists($ruta)){
				mkdir($ruta,0777,true);
			}
			if (move_uploaded_file($tmp, $ruta.$nombre.".".$tipo)) {
				$update4 = $myPDO2->prepare("
			   		UPDATE notas SET file4='$nombre.$tipo' 
					WHERE id_estudiante = $alumno AND id_plan_evaluacion = $plan;
			   		");
			   	$update4->execute();
				#print "ARCHIVO GUARDADO";
			}else{
				#print "NO SE GUARDO EL ARCHIVO";
			}
		}
		
	}
		

		

	// print "alumno:".$alumno." - materia:".$materia." - plan:".$plan." - evalu:".$evalu." - nota:".$nota." - obs:".$observacion;
	// exit;
	
	if ($insertar OR $update OR  $update1 OR $update2 OR $update3 OR $update4) {
		if ($toggle == 1) {
			header('location: ../../views/CargarNota/cargar.php?mat='.$materia.'&plan='.$plan.'&gu=t');
		}else{
			header('location: ../../views/Evaluaciones/evaluacionDetalles.php?mat='.$materia.'&evalu='.$evalu.'&gu=t');
		}
		
		
	} else {
		if ($toggle == 1) {
			header('location: ../../views/CargarNota/cargar.php?mat='.$materia.'&plan='.$plan.'&gu=f');
		}else{
			header('location: ../../views/Evaluaciones/evaluacionDetalles.php?mat='.$materia.'&evalu='.$evalu.'&gu=f');
		}
		
		
	}

	









?>